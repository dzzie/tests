...
Private Sub BuildShards()
    ' v1-style irregular shards, but with full coverage. For outer rings, we
    ' generate BOTH the apex-out and apex-in triangles for every segment
    ' (v1 only built one per segment, alternating, which left slivers of
    ' intact desktop visible between them). With both built, the impact zone
    ' is fully covered by actual shard polygons that the fist can clear.
    ' Adjacent triangles overlap slightly, which preserves the chaotic
    ' "real broken glass" look the v1 pattern is known for.
    ...
    ' Ring 0 contributes ringSegs(0) triangles. Each outer ring contributes
    ' ringSegs(i) * 2 (both orientations per segment).
    Dim total As Long
    total = ringSegs(0)
    For i = 1 To 4
    ...
    For i = 0 To 4
        ...
        For j = 0 To segCount - 1
            ...
            Else
                ...
                ' Apex-out triangle: 1 vertex on inner arc, 2 vertices on outer arc
                idx = m_nShards + 1
                m_nShards = idx
                jitter = (Rnd - 0.5) * 20
                ...
                ' Apex-in triangle: 2 vertices on inner arc, 1 vertex on outer arc.
                ' Independent jitter so the two pieces look distinct.
                idx = m_nShards + 1
                m_nShards = idx
                jitter = (Rnd - 0.5) * 20
