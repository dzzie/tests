...
Private Const FR_FIST_HOLD = 18     ' beat at max — held debris flies outward, readable
Private Const FR_FIST_PULLBACK = 30 ' retreat into void as remaining debris clears
...
Private Sub BuildShards()
    ' v1-style irregular shards (variable segment count per ring), but with
    ' within-ring shared geometry to eliminate the gap-slivers between
    ' adjacent shards. For each ring we precompute jittered ray angles and
    ' per-ray inner/outer radii once, so every triangle in that ring shares
    ' its boundary edges with neighbors exactly.
    ...
    Dim maxSegs As Long
    maxSegs = 18  ' = ringSegs(4)

    ' Per-ring scratch arrays — recomputed each ring since segment counts vary.
    Dim rayAng(0 To 18) As Single
    Dim rOuter(0 To 18) As Single  ' radius at each ray (outer edge of ring)
    ...
    For i = 0 To 4
        ...
        ' Precompute shared ray angles and the jittered ring radius at each ray
        For j = 0 To segCount
            rayAng(j) = (j / segCount) * 6.2831853
            If j > 0 And j < segCount Then
                rayAng(j) = rayAng(j) + (Rnd - 0.5) * 0.15
            End If
            rOuter(j) = rOut + (Rnd - 0.5) * 20
        Next j
        rayAng(segCount) = rayAng(0) + 6.2831853  ' wrap
        rOuter(segCount) = rOuter(0)

        For j = 0 To segCount - 1
            Dim a1 As Single, a2 As Single, aMid As Single
            a1 = rayAng(j)
            a2 = rayAng(j + 1)
            aMid = (a1 + a2) * 0.5

            If i = 0 Then
                ' Innermost ring: single wedge with apex at center.
                ' Adjacent wedges share their ray edges via rayAng/rOuter.
                idx = m_nShards + 1
                m_nShards = idx
                m_shards(idx).p0x = m_cx
                m_shards(idx).p0y = m_cy
                m_shards(idx).p1x = m_cx + Cos(a1) * rOuter(j)
                m_shards(idx).p1y = m_cy + Sin(a1) * rOuter(j)
                m_shards(idx).p2x = m_cx + Cos(a2) * rOuter(j + 1)
                m_shards(idx).p2y = m_cy + Sin(a2) * rOuter(j + 1)
                FinalizeShard idx
            Else
                ' Outer rings: build BOTH apex orientations per segment.
                ' Both share the same rayAng + rOuter values, so they
                ' meet exactly at the segment's ray edges.
                Dim rInJit As Single
                rInJit = rIn + (Rnd - 0.5) * 15  ' small radial jitter for the apex point

                ' Apex-out: 1 vertex on inner mid, 2 vertices on outer ring
                idx = m_nShards + 1
                m_nShards = idx
                m_shards(idx).p0x = m_cx + Cos(aMid) * rInJit
                m_shards(idx).p0y = m_cy + Sin(aMid) * rInJit
                m_shards(idx).p1x = m_cx + Cos(a1) * rOuter(j)
                m_shards(idx).p1y = m_cy + Sin(a1) * rOuter(j)
                m_shards(idx).p2x = m_cx + Cos(a2) * rOuter(j + 1)
                m_shards(idx).p2y = m_cy + Sin(a2) * rOuter(j + 1)
                ...
                ' Apex-in: 2 vertices on inner ring, 1 vertex on outer mid
                Dim rOutJit As Single
                rOutJit = rOut + (Rnd - 0.5) * 15
                idx = m_nShards + 1
                m_nShards = idx
                m_shards(idx).p0x = m_cx + Cos(aMid) * rOutJit
                m_shards(idx).p0y = m_cy + Sin(aMid) * rOutJit
                m_shards(idx).p1x = m_cx + Cos(a1) * rIn
                m_shards(idx).p1y = m_cy + Sin(a1) * rIn
                m_shards(idx).p2x = m_cx + Cos(a2) * rIn
                m_shards(idx).p2y = m_cy + Sin(a2) * rIn
                FinalizeShard idx
            End If
        Next j
    Next i
...
Private Sub BuildCracks()
    ...
    ' Shuffle so DrawCracks reveals them in random order rather than going
    ' clockwise ray-by-ray. Fisher-Yates: walk back-to-front, swap each
    ' element with a random earlier element.
    Dim s As Long, tmpC As CrackLine
    For i = m_nCracks To 2 Step -1
        s = 1 + Int(Rnd * i)  ' 1..i inclusive
        If s <> i Then
            tmpC = m_cracks(i)
            m_cracks(i) = m_cracks(s)
            m_cracks(s) = tmpC
        End If
    Next i
End Sub
...
Private Sub tmr_Timer()
    ...
    Select Case m_phase
        ...
        Case PHASE_FIST_STRIKE
            ...
            RenderShatterFrame loc, 0.85, strikeScale
        ...
        Case PHASE_FIST_PULLBACK
            ...
            RenderShatterFrame loc, 0.85, pullScale
...
'
Private Sub RenderShatterFrame(ByVal localFr As Long, ByVal demonProgress As Single, Optional ByVal fistScale As Single = 0)
    ...
    ' --- Layer 2b: fist (if present). Drawn AFTER the demon but BEFORE the
    '     desktop layer, so the held shards naturally occlude it. The fist
    '     reads as "behind the glass, coming forward" — visible through the
    '     central hole but hidden by any still-attached shard.
    If fistScale > 0 Then DrawFist fistScale
...
Private Sub ExplodeHolders()
    ...
    For i = 1 To m_nShards
        If Not m_shards(i).detached Then
            ...
            outAng = Atan2Approx(ddy, ddx)
            kick = 5 + Rnd * 4   ' 5..9 px/frame radial — visible as debris
