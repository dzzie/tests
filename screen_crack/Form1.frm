...
Private Const PHASE_FINAL = 3
Private Const PHASE_DONE = 4
...
Private Const FR_SHATTER = 110  ' shards detach in waves and fall
...
Private Sub BuildShards()
    ...
    For i = 0 To 4
        ...
        For j = 0 To segCount - 1
            ...
            ' Detachment timing: closer to center = falls sooner.
            ' Map distance 0..600 -> frames 0..(FR_SHATTER - 40), with some jitter.
            Dim base As Single
            base = (m_shards(idx).distFromImpact / 600) * (FR_SHATTER - 40)
            ...
            ' Falling velocity: mostly down, slight outward bias from impact
            Dim outAng As Single
            outAng = Atan2Approx(ddy, ddx)
            m_shards(idx).vx = Cos(outAng) * (0.5 + Rnd * 1.5)
            m_shards(idx).vy = 0.5 + Rnd * 1.2  ' initial down kick; gravity adds
...
'
Private Sub RenderShatterFrame(ByVal localFr As Long)
    UpdateShardPhysics localFr

    ' --- Layer 1: black void ---
    PatBlt Me.hDC, 0, 0, m_scrW, m_scrH, BLACKNESS

    ' --- Layer 2: demon lurking, getting brighter over the phase ---
    Dim shardProgress As Single
    shardProgress = localFr / FR_SHATTER
    If shardProgress > 1 Then shardProgress = 1
    DrawLurkingDemon shardProgress
    ...
    ' Once all shards have fallen well off-screen, advance.
    ' Cheap check: when local frame past shard count threshold.
    If localFr >= FR_SHATTER Then NextPhase PHASE_FINAL
End Sub
...
Private Sub DrawCracks(ByVal localFr As Long, ByVal totalFr As Long)
    ...
    penDark = CreatePen(PS_SOLID, 3, RGB(10, 10, 12))
    penLight = CreatePen(PS_SOLID, 1, RGB(235, 235, 240))
...
'    t = localFr / totalFr
'    If t > 1 Then t = 1
'    ' Wide light halo first, black center line on top.
'    ' Halo is subtle so it reads as "compressed glass around the fracture"
'    ' rather than a glow.
'    Dim penHalo As Long, penCore As Long, oldPen As Long
'    penHalo = CreatePen(PS_SOLID, 3, RGB(210, 210, 215))
'    penCore = CreatePen(PS_SOLID, 1, RGB(0, 0, 0))
'    Dim i As Long, drawTo As Long
'    drawTo = Int(m_nCracks * t)
'    If drawTo < 1 Then
'        DeleteObject penHalo: DeleteObject penCore
'        Exit Sub
'    End If
'    oldPen = SelectObject(Me.hDC, penHalo)
'    For i = 1 To drawTo
'        MoveToEx Me.hDC, m_cracks(i).x1, m_cracks(i).y1, 0
'        LineTo Me.hDC, m_cracks(i).x2, m_cracks(i).y2
'    Next i
'    SelectObject Me.hDC, penCore
'    For i = 1 To drawTo
'        MoveToEx Me.hDC, m_cracks(i).x1, m_cracks(i).y1, 0
'        LineTo Me.hDC, m_cracks(i).x2, m_cracks(i).y2
'    Next i
