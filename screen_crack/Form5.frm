...
Private Sub BuildShards()
    ...
    For i = 1 To N_RINGS - 1
        Dim isOutermost As Boolean
        isOutermost = (i = N_RINGS - 1)
        For k = 0 To SEGS_PER_RING - 1
            ' For the outermost ring, ~30% of quads hold on — both triangles
            ' in the quad get marked together so we don't end up with half-
            ' quads dangling, which looks fragmented.
            Dim segHolds As Boolean
            segHolds = False
            If isOutermost Then segHolds = (Rnd < 0.6)
            ...
            FinalizeShard m_nShards
            m_shards(m_nShards).holdsOn = segHolds
    ...
    Select Case m_phase
        ...
        Case PHASE_PAUSE
            ...
            If loc >= FR_PAUSE Then
                ...
                ' holdsOn shards are skipped so the ragged edge survives.
                Dim k As Long
                For k = 1 To m_nShards
                    If (Not m_shards(k).detached) And (Not m_shards(k).holdsOn) Then
