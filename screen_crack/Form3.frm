...
' The original radiating-wave timing reaches detachFrame ~70 for the outermost
' shards. We cut PHASE_SHATTER short at frame 40 so only the inner portion of
' the wave plays. On entry to PHASE_SHATTER2 we rewrite the remaining shards'
' detachFrames to small values so they detach quickly during the second wave —
' visually it's the same single wave with a pause carved out of the middle.
Private Const FR_CRACK = 25
Private Const FR_SHATTER = 40   ' wave 1: inner portion of original wave
Private Const FR_PAUSE = 25     ' beat between waves (~0.4s) — demon glimpse
Private Const FR_SHATTER2 = 90  ' wave 2: remaining shards detach and fall
...
Private Sub BuildShards()
    ...
    For i = 0 To 4
        ...
        For j = 0 To segCount - 1
            ...
            ' Single radiating wave: detachFrame scales linearly with distance
            ' from impact (innermost shards go first, outer last) — same math as
            ' the original implementation. PHASE_SHATTER is intentionally cut
            ' short below so only the inner portion of this wave plays. The
            ' remaining shards get their detachFrames rewritten when we
            ' transition into PHASE_SHATTER2, so the second wave continues
            ...
            m_shards(idx).wave = 0  ' unused now but kept on the type

            ' Falling velocity (original): mostly down, slight outward bias from impact
...
Private Sub tmr_Timer()
    ...
    Select Case m_phase
        ...
        Case PHASE_PAUSE
            ' No new detachments (localFr in this phase starts at 0, well below
            ' any remaining shard's detachFrame which are all >= ~40).
            ' Already-detached shards continue falling. Demon visible through hole.
            Dim pauseProg As Single
            pauseProg = loc / FR_PAUSE
            If pauseProg > 1 Then pauseProg = 1
            RenderShatterFrame loc, 0.35 + pauseProg * 0.2
            If loc >= FR_PAUSE Then
                PlaySecondCrack
                ' Rewrite remaining shards' detachFrames so they detach quickly
                ' in PHASE_SHATTER2 — this is the "resume the wave" moment.
                Dim k As Long
                For k = 1 To m_nShards
                    If Not m_shards(k).detached Then
                        m_shards(k).detachFrame = CLng(Rnd * 15)
                    End If
                Next k
        ...
        Case PHASE_SHATTER2
            ...
            RenderShatterFrame loc, 0.55 + w2Prog * 0.3
